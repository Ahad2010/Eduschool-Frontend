...
export default function Register() {
  ...
  const [form,    setForm]    = useState({
    ...
    role:"student", class:"", subject:"", qualification:"",
  });
  ...
  const handleNext = (e) => { e.preventDefault(); if (validate1()) setStep(2); };
    ...
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#0d1117,#1a1f37)", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
      <div style={{ textAlign:"center", padding:48, maxWidth:420 }}>
        ...
        <p style={{ color:"#475569", fontSize:14, lineHeight:1.8, margin:"0 0 28px" }}>
          Your registration is pending <span style={{ color:"#fbbf24", fontWeight:700 }}>Admin Approval</span>. You'll receive access once approved.
        </p>
        ...
        <form onSubmit={step===1?handleNext:handleSubmit}>
          {step===1 && <>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <input placeholder="Enter full name" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} style={inp(errors.name)}
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <input type="email" placeholder="Enter email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} style={inp(errors.email)}
            ...
            {/* Phone */}
            <div style={{ marginBottom:14 }}>
              <label style={{ display:"block", color:"#94a3b8", fontSize:12, fontWeight:600, marginBottom:6 }}>Phone</label>
              <input placeholder="03XX-XXXXXXX" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} style={inp(errors.phone)}
            ...
            {/* Subject + Qualification (teacher only) */}
            {form.role==="teacher" && <>
              <div style={{ marginBottom:14 }}>
                <label style={{ display:"block", color:"#94a3b8", fontSize:12, fontWeight:600, marginBottom:6 }}>Subject</label>
                <input placeholder="e.g. Mathematics" value={form.subject} onChange={e=>setForm({...form,subject:e.target.value})} style={inp(false)}
                  onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#21293a"}/>
              </div>
              <div style={{ marginBottom:14 }}>
                <label style={{ display:"block", color:"#94a3b8", fontSize:12, fontWeight:600, marginBottom:6 }}>Qualification</label>
                <input placeholder="e.g. M.Sc Physics" value={form.qualification} onChange={e=>setForm({...form,qualification:e.target.value})} style={inp(false)}
                  onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#21293a"}/>
              </div>
            </>}
          ...
          {step===2 && <>
            ...
            <div style={{ background:"rgba(251,191,36,0.08)", border:"1px solid rgba(251,191,36,0.2)", borderRadius:9, padding:"10px 14px", marginBottom:18 }}>
              <p style={{ color:"#fbbf24", fontSize:12, margin:0 }}>
                ⏳ Account needs <strong>Admin Approval</strong> before login.
              </p>
            </div>
